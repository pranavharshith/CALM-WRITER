...
router.get('/:hubId', optionalAuth, async (req, res) => {
  try {
    ...
    const isMember = !!(currentMember && currentMember.isActive !== false);
    const isModerator = !!(currentMember && (currentMember.role === 'creator' || currentMember.role === 'moderator') && currentMember.isActive !== false);
    ...
    res.json({
      ...
      hub: {
        ...
        lastChatMessageAt: hub.lastChatMessageAt,
        unreadChat: !!(hub.lastChatMessageAt && currentMember
          && (!currentMember.lastSeenAt || hub.lastChatMessageAt > currentMember.lastSeenAt)),
