...
// GET /leaderboards/quietly-powerful
// Substantial writing that didn't go viral — depth over popularity.
router.get('/quietly-powerful', optionalAuth, async (req, res) => {
  try {
    ...
    const stories = await Story.aggregate([
      ...
      },
      {
        $addFields: {
          wc: {
            $cond: [
              { $gt: [{ $ifNull: ['$wordCount', 0] }, 0] },
              '$wordCount',
              {
                $size: {
                  $filter: {
                    input: { $split: [{ $trim: { input: { $ifNull: ['$text', ''] } } }, ' '] },
                    as: 'w',
          ...
          likeCount: { $max: [0, { $ifNull: ['$likes', 0] }] }
        }
      },
      { $match: { wc: { $gte: 1 } } },
      {
      ...
      {
        $addFields: {
          reactionsCount: { $ifNull: [{ $arrayElemAt: ['$rx.n', 0] }, 0] }
        }
      },
      {
        $addFields: {
          quietScore: {
            $divide: [
              { $add: [{ $divide: ['$wc', 80] }, { $multiply: ['$reactionsCount', 5] }] },
              { $add: [1, '$likeCount'] }
            ]
          }
        }
      },
      { $sort: { quietScore: -1, wc: -1, createdAt: -1 } },
      { $limit: limit },
      {
        $lookup: {
          from: 'users',
    ...
    res.json({
      success: true,
      stories,
      description: 'Longer stories that linger without chasing likes'
    });
  } catch (error) {
